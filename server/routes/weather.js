...
router.get('/current', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Server error fetching current weather', details: err.message })
  }
})
...
router.get('/forecast', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Server error fetching forecast', details: err.message })
  }
})
...
router.get('/history', async (req, res) => {
  try {
    ...
    if (!q || !date) return res.status(400).json({ error: 'Missing city name or date' })
    ...
    res.status(500).json({ error: 'Server error fetching history', details: err.message })
  }
})
...
router.get('/search', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Server error fetching search results', details: err.message })
  }
})
